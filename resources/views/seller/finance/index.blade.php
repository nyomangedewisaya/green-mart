...
@section('content')
    ...
    {{-- 1. PEMBUKA X-DATA (Mencakup SELURUH konten) --}}
    <div x-data="{
        withdrawModal: false,
        amount: '',
        ...
        openTrxModal(trx) {
            ...
            console.log('Opening modal with:', trx);
            this.activeTrx = trx;
            this.transactionModal = true;
        },
    
    }" class="space-y-6 font-inter relative">
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($transactions as $trx)
                            {{-- PERBAIKAN DI SINI: Ganti kutip dua " dengan kutip satu ' dan pakai @json --}}
                            <tr @click='openTrxModal(@json($trx))'
                                ...
                                <td class="px-6 py-4 align-top">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wide {{ $trx->type == 'income' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200' }}">
                                ...
                                <td class="px-6 py-4 text-center align-top">
                                    @if ($trx->description == 'promosi')
                                        @if ($trx->status == 'pending')
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold bg-yellow-100 text-yellow-700 border border-yellow-200 uppercase"><span
                                                    class="w-1.5 h-1.5 rounded-full bg-yellow-500 mr-1.5 animate-pulse"></span>
                                                Review</span>
                                        @elseif($trx->status == 'paid')
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold bg-purple-50 text-purple-700 border border-purple-200 uppercase"><span
                                                    class="w-1.5 h-1.5 rounded-full bg-purple-500 mr-1.5"></span>
                                                Aktif</span>
                                        @else
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200 uppercase">Selesai</span>
                                        @endif
                                    @elseif($trx->type == 'expense')
                                        @if ($trx->description == 'pending')
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold bg-yellow-50 text-yellow-700 border border-yellow-200 uppercase"><span
                                                    class="w-1.5 h-1.5 rounded-full bg-yellow-500 mr-1.5 animate-pulse"></span>
                                                Proses</span>
                                        @elseif($trx->description == 'approved')
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold bg-green-50 text-green-700 border border-green-200 uppercase"><span
                                                    class="w-1.5 h-1.5 rounded-full bg-green-500 mr-1.5"></span>
                                                Sukses</span>
                                        @else
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold bg-red-50 text-red-700 border border-red-200 uppercase"><span
                                                    class="w-1.5 h-1.5 rounded-full bg-red-500 mr-1.5"></span> Gagal</span>
                                        @endif
                                    @else
        ...
        <div x-show="withdrawModal" class="fixed inset-0 z-100 flex items-center justify-center p-4"
            ...
            <div x-show="withdrawModal"
                ...
                <div class="bg-green-600 px-6 py-5 flex justify-between items-center text-white shadow-md z-10">
                    <h3 class="text-lg font-bold">Tarik Saldo</h3>
                ...
                <form action="{{ route('seller.finance.withdraw') }}" method="POST"
                    class="p-6 space-y-5 overflow-y-auto custom-scroll" x-data="{
                        ...
                        fillBank(el) {
                            ...
                            if (option.value) {
                                this.selectedBankId = option.value;
                        ...
                        deleteNow() {
                            if (this.selectedBankId) {
                                document.getElementById('delete-bank-' + this.selectedBankId).submit();
                            }
                        }
                    }">
                    @csrf
                    ...
                    <div
                        class="bg-green-50 p-4 rounded-xl border border-green-100 flex justify-between items-center shadow-sm">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="amount" x-model="amount" required min="10000"
                                max="{{ $currentBalance }}"
                                class="w-full pl-10 pr-16 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none font-bold text-gray-800 text-lg transition shadow-sm"
                                placeholder="0">
                            <button type="button" @click="setMaxAmount({{ $currentBalance }})"
                                class="absolute right-3 top-3.5 text-xs bg-green-100 text-green-700 px-2 py-1.5 rounded hover:bg-green-200 font-bold transition">MAX</button>
                        </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-3">
                            <label class="text-xs font-bold text-gray-500 uppercase">Rekening Tujuan</label>
                        </div>

                        @if ($savedBanks->count() > 0)
                            <div class="bg-gray-100 p-1 rounded-xl flex mb-4 shadow-inner">
                                <button type="button" @click="useSaved = true"
                                    class="flex-1 py-2 text-xs font-bold rounded-lg transition text-center"
                                    :class="useSaved ? 'bg-white text-green-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'">
                                    Pilih Tersimpan
                                </button>
                                <button type="button" @click="useSaved = false"
                                    class="flex-1 py-2 text-xs font-bold rounded-lg transition text-center"
                                    :class="!useSaved ? 'bg-white text-green-600 shadow-sm' :
                        ...
                        <div x-show="useSaved && {{ $savedBanks->count() > 0 ? 'true' : 'false' }}" class="space-y-4"
                            x-transition>
                            <div class="relative">
                                <select @change="fillBank($event.target)"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 outline-none text-sm bg-white appearance-none cursor-pointer shadow-sm">
                            ...
                            <div x-show="selectedBankId"
                                class="relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-700 to-slate-800 text-white p-5 shadow-lg transition transform"
                                x-transition>
                                <div
                                    class="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full blur-2xl -mr-10 -mt-10">
                                </div>
                                <button type="button" @click="deleteNow()"
                                    class="absolute top-3 right-3 p-2 bg-white/10 hover:bg-red-500 rounded-lg text-white/70 hover:text-white transition backdrop-blur-sm z-20 group"
                                ...
                                <div class="flex justify-between items-start mb-6 relative z-10">
                                    <div class="text-xs font-bold bg-white/20 px-2 py-1 rounded text-white/90 border border-white/10 uppercase tracking-wide"
                                        x-text="bankName"></div>
                                </div>
                                <div class="relative z-10">
                                    <div class="font-mono text-xl tracking-widest mb-1 text-white shadow-sm"
                                        x-text="accNum"></div>
                                    <div class="text-xs text-gray-300 uppercase font-medium tracking-wider"
                                        x-text="accHolder"></div>
                                </div>
                            </div>
                        </div>

                        <div x-show="!useSaved || {{ $savedBanks->count() == 0 ? 'true' : 'false' }}" class="space-y-3"
                            x-transition>
                            <div>
                                ...
                                <input type="text" name="bank_name" x-model="bankName" :required="!useSaved"
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 outline-none text-sm transition placeholder-gray-400"
                            ...
                            <label
                                ...
                                <span class="text-xs text-gray-600 font-medium">Simpan rekening ini ke daftar saya</span>
                            </label>
                        </div>
                    </div>
        ...
        <div x-show="transactionModal" class="fixed inset-0 z-100 flex items-center justify-center p-4"
            ...
            {{-- Backdrop --}}
            <div x-show="transactionModal" @click="transactionModal = false" x-transition:enter="ease-out duration-300"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-900/75 backdrop-blur-sm">
            </div>
            ...
            <div x-show="transactionModal"
                ...
                <div class="px-6 py-6 text-white relative overflow-hidden shrink-0"
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-4 pt-2">
                                    <div>
                                        ...
                                        {{-- PERBAIKAN DI SINI: Ganti formatDate jadi formatDateOnly --}}
                                        <p class="font-medium text-sm text-gray-800"
                                            x-text="formatDateOnly(activeTrx?.start_date)"></p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-gray-500 mb-1">Selesai Tayang</p>
                                        {{-- PERBAIKAN DI SINI: Ganti formatDate jadi formatDateOnly --}}
                                        <p class="font-medium text-sm text-gray-800"
                                            x-text="formatDateOnly(activeTrx?.end_date)"></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <template
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="bg-white border border-gray-200 rounded-xl p-4 space-y-3 shadow-sm">
                                <div class="flex justify-between items-center border-b border-gray-100 pb-2"><span
                                        class="text-xs text-gray-500 uppercase font-bold">Bank Tujuan</span><span
                                        class="font-bold text-gray-900" x-text="activeTrx?.bank_name"></span></div>
                                <div class="flex justify-between items-center border-b border-gray-100 pb-2"><span
                                        class="text-xs text-gray-500 uppercase font-bold">No. Rekening</span><span
                                        class="font-mono font-medium text-gray-800 select-all"
                                        x-text="activeTrx?.account_number"></span></div>
                                <div class="flex justify-between items-center"><span
                                        class="text-xs text-gray-500 uppercase font-bold">Atas Nama</span><span
                                        class="font-medium text-gray-800 uppercase"
                                        x-text="activeTrx?.account_holder"></span></div>
                            </div>
                            <template x-if="activeTrx?.description == 'rejected' && activeTrx?.note">
                                <div class="bg-red-50 border border-red-200 p-3 rounded-xl text-sm">
                                    ...
                                    <p class="text-red-600 leading-relaxed">"<span x-text="activeTrx?.note"></span>"</p>
                                </div>
                            </template>
                        </div>
                    </template>
                ...
                <div class="bg-gray-50 px-6 py-4 border-t border-gray-100 text-center shrink-0">
                    <button @click="transactionModal = false"
                        class="text-sm font-bold text-gray-500 hover:text-gray-800 transition">Tutup Detail</button>
                </div>

            </div>
        </div>

        {{-- PENUTUP X-DATA X-DATA --}}
    </div>
