...
@section('content')
    <div x-data="{
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            {{-- 1. KARTU SLOT MONITOR --}}
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 lg:col-span-1">
                <div class="flex justify-between items-center mb-2">
                    <h3 class="font-bold text-gray-800">Ketersediaan Slot</h3>
                    <span class="text-xs font-bold px-2 py-1 rounded bg-gray-100 text-gray-600">Max {{ $maxSlots }}</span>
                </div>
                
                <div class="flex items-end gap-2 mb-3">
                    <span class="text-4xl font-bold {{ $activeSlots >= $maxSlots ? 'text-red-600' : 'text-green-600' }}">
                        {{ $activeSlots }}
                    </span>
                    <span class="text-gray-400 text-sm mb-1">/ {{ $maxSlots }} Iklan Aktif</span>
                </div>

                <div class="w-full bg-gray-100 rounded-full h-2.5 mb-2 overflow-hidden">
                    <div class="h-2.5 rounded-full {{ $activeSlots >= $maxSlots ? 'bg-red-500' : 'bg-green-500' }}" style="width: {{ $slotPercentage }}%"></div>
                </div>
                <p class="text-xs text-gray-500">
                    @if($activeSlots >= $maxSlots)
                        <span class="text-red-500 font-bold">SLOT PENUH!</span> Seller tidak bisa mengajukan iklan baru.
                    @else
                        Tersedia <strong>{{ $maxSlots - $activeSlots }}</strong> slot lagi.
                    @endif
                </p>
            </div>

            {{-- 2. KARTU ANTREAN PENDING (KHUSUS) --}}
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 lg:col-span-2">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-xs font-bold bg-yellow-100 text-yellow-800 px-2 py-1 rounded-lg">{{ $pendingPromotions->count() }} Request</span>
                </div>

                @if($pendingPromotions->count() > 0)
                    <div class="space-y-3 max-h-[140px] overflow-y-auto custom-scroll pr-2">
                        @foreach($pendingPromotions as $promo)
                            <div class="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-100">
                                <div class="flex items-center gap-3">
                                    <img src="{{ asset($promo->image) }}" class="w-10 h-10 rounded object-cover bg-white">
                                    <div>
                                        <p class="text-sm font-bold text-gray-800 line-clamp-1">{{ $promo->title }}</p>
                                        <p class="text-xs text-gray-500">{{ $promo->seller->name }} • {{ number_format($promo->price) }}</p>
                                    </div>
                                </div>
                                <button @click="statusModal = true; statusForm.title = '{{ $promo->title }}'; statusForm.action = '{{ route('admin.promotions.update', $promo) }}';"
                                    class="px-3 py-1.5 bg-white text-green-600 border border-green-200 rounded-md text-xs font-bold hover:bg-green-50 shadow-sm transition">
                                    Proses
                                </button>
                            </div>
                        @endforeach
                ...
                @else
                    <div class="flex flex-col items-center justify-center h-[120px] text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                        <svg class="w-8 h-8 mb-2 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        <span class="text-sm">Semua aman! Tidak ada antrean.</span>
                    </div>
                @endif
            </div>
        </div>
