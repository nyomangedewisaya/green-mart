            ...
            <div
                ...
                <div class="relative z-10 grid grid-cols-2 gap-3 flex-1">
                    <a href="{{ route('admin.withdrawals.index', ['status' => 'pending']) }}"
                        class="flex flex-col justify-center items-center p-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl transition">
                        <span
                            class="text-xl font-bold {{ $pendingWithdrawals > 0 ? 'text-yellow-400' : 'text-gray-500' }}">{{ $pendingWithdrawals }}</span>
                        <span class="text-xs text-gray-400 mt-1">Penarikan</span>
                    </a>
                    <a href="{{ route('admin.verification.index') }}"
                        class="flex flex-col justify-center items-center p-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl transition">
                        <span
                            class="text-xl font-bold {{ $pendingSellers > 0 ? 'text-blue-400' : 'text-gray-500' }}">{{ $pendingSellers }}</span>
                        <span class="text-xs text-gray-400 mt-1">Verifikasi</span>
                    </a>
                    <a href="{{ route('admin.reports.index', ['status' => 'pending']) }}"
                        ...
                        <span class="text-xs text-gray-400 mt-1">Laporan</span>
                    </a>
                    <a href="{{ route('admin.transactions.index', ['status' => 'pending']) }}"
                        class="flex flex-col justify-center items-center p-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl transition">
                        <span
                            class="text-xl font-bold {{ $pendingOrders > 0 ? 'text-orange-400' : 'text-gray-500' }}">{{ $pendingOrders }}</span>
                        <span class="text-xs text-gray-400 mt-1">Unpaid</span>
                    </a>
                </div>
            </div>
        </div>
                            ...
                            @forelse($topProducts as $item)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-4 py-3 text-right"><span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold bg-green-100 text-green-800">{{ $item->total_sold }}
                                            Terjual</span></td>
                                </tr>
                            @empty
                                <tr>
