...
@section('content')
    ...
    <div x-data="{
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div
                class="bg-white p-6 rounded-xl shadow-sm border border-yellow-100 flex items-center justify-between hover:shadow-md transition">
                <div>
            ...
            <div
                class="bg-white p-6 rounded-xl shadow-sm border border-green-100 flex items-center justify-between hover:shadow-md transition">
                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div class="md:col-span-5">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" placeholder="Nama seller, bank..." x-model="searchQuery"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="per_page" x-ref="perPageSelect">
                            @foreach ([10, 25, 50, 100] as $num)
                                <option value="{{ $num }}" @selected(request('per_page', 10) == $num)>
                                    {{ $num }} data
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-max">
                    <thead class="border-b border-gray-200">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Tanggal Request
                            </th>
                        ...
                        @forelse($withdrawals as $wd)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-4 text-sm text-gray-600">
                                    {{ $wd->created_at->locale('id')->translatedFormat('d M Y, H:i') }} WIB
                                </td>
                                <td class="px-4 py-4 text-sm font-medium text-gray-900">{{ $wd->seller->name ?? 'Unknown' }}
                                </td>
                                <td class="px-4 py-4 text-sm text-gray-700">
                                    <div class="font-bold text-gray-800">{{ $wd->bank_name }}</div>
                                    <div class="text-xs text-gray-500 font-mono">{{ $wd->account_number }}</div>
                                    <div class="text-xs text-gray-500 uppercase truncate max-w-[150px]">
                                        {{ $wd->account_holder }}</div>
                                </td>
                                ...
                                <td class="px-4 py-4 text-center">
                                    <button
                                        @click="detailModal = true; modalData = {{ $wd->toJson() }}; modalData.seller_name = '{{ $wd->seller->name ?? '' }}';"
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-12">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm text-gray-500 mt-1">Tidak ada permintaan penarikan yang sesuai.
                                        </p>
                                    </div>
                                </td>
                            </tr>
        ...
        <div x-show="detailModal" class="fixed inset-0 z-100 flex items-center justify-center p-4"
            ...
            <div x-show="detailModal" class="relative w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden"
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="text-center mb-4">
                        ...
                        <h2 class="text-3xl font-bold text-gray-800 mt-1" x-text="formatCurrency(modalData?.amount)"></h2>
                        <div
                            class="mt-2 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-600 border">
                            <span x-text="modalData?.seller_name"></span>
                        </div>
                    </div>

                    <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 shadow-sm">
                        <h4 class="text-xs font-bold text-blue-500 uppercase tracking-wider mb-3">Rekening Tujuan</h4>
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div>
                                ...
                                <p class="font-mono font-bold text-gray-800 text-lg tracking-wide select-all"
                                    x-text="modalData?.account_number"></p>
                            </div>
                    ...
                    <form :action="'{{ url('admin/withdrawals') }}/' + modalData?.id" method="POST" x-ref="approveForm"
                        class="hidden">
                        @csrf @method('PUT')
                        <input type="hidden" name="action" value="approve">
                    </form>

                    <form :action="'{{ url('admin/withdrawals') }}/' + modalData?.id" method="POST" x-ref="rejectForm"
                        class="hidden">
                        @csrf @method('PUT')
                    ...
                    <div x-show="modalData?.status == 'pending'" class="pt-2">
                        <label class="text-xs font-bold text-gray-500">Catatan Admin (Wajib jika menolak)</label>
                        <textarea x-model="rejectReason"
                            class="w-full mt-1 p-3 text-sm border border-gray-300 rounded-lg focus:ring-1 focus:ring-red-500 outline-none resize-none"
                            rows="2" placeholder="Alasan penolakan..."></textarea>
                    </div>
                </div>

                <div class="bg-gray-50 px-6 py-4 border-t border-gray-200" x-show="modalData?.status == 'pending'">
                    <div class="grid grid-cols-2 gap-3">
                        <button @click="confirmType = 'reject'; confirmModal = true" :disabled="!rejectReason"
                ...
                <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 text-center"
                    x-show="modalData?.status != 'pending'">
                    <p class="text-sm text-gray-500">Transaksi ini telah diproses pada <span class="font-bold"
                            x-text="formatDate(modalData?.updated_at)"></span></p>
                    <p class="text-xs text-gray-400 mt-1" x-show="modalData?.admin_note">Catatan: <span
                            x-text="modalData?.admin_note"></span></p>
                </div>
            </div>
        </div>

        <div x-show="confirmModal" class="fixed inset-0 z-[110] flex items-center justify-center p-4"
            style="display: none;">
            <div x-show="confirmModal" @click="confirmModal = false"
                class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm" x-transition:enter="ease-out duration-200"
            ...
            <div x-show="confirmModal" class="relative w-full max-w-sm bg-white rounded-xl shadow-2xl overflow-hidden"
                ...
                <div class="p-6 text-center">
                    <div class="mx-auto flex items-center justify-center h-14 w-14 rounded-full mb-4"
                        ...
                        <svg x-show="confirmType === 'approve'" class="h-8 w-8 text-green-600" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <svg x-show="confirmType === 'reject'" class="h-8 w-8 text-red-600" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <h3 class="text-xl font-bold text-gray-900"
                        x-text="confirmType === 'approve' ? 'Konfirmasi Transfer' : 'Konfirmasi Penolakan'"></h3>

                    <p class="text-sm text-gray-500 mt-2" x-show="confirmType === 'approve'">
                        Apakah Anda yakin <strong>sudah melakukan transfer</strong> uang sejumlah tersebut ke rekening
                        seller? Aksi ini tidak dapat dibatalkan.
                    </p>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 flex flex-row-reverse gap-2">
                    <button @click="submitAction()" type="button"
                        ...
                        :class="confirmType === 'approve' ? 'bg-green-600 hover:bg-green-700 focus:ring-green-500' :
